  ...
  async function saveEfetivo(date, obra) {
    ...
    const workers = s.workers.filter(w => w.obra === obra).map(w => {
      ...
      return {nome:w.nome, funcao:w.funcao, equipe:eq?eq.nome:w.equipe,
              presente:dd.presente!==false, andar:dd.andar||'', tarefas:dd.tarefas||[], motivo:dd.motivo||''};
    });
...
const Versoes = (() => {
  const LISTA = [
    ...
    },
    {
      ...
      notas: [
        {tipo:'correcao', texto:'O motivo da falta (atestado, consulta, etc.) nunca era enviado à planilha e voltava sempre vazio na sincronização — sumia até no próprio aparelho na releitura seguinte. A planilha ganhou a coluna "Motivo da Falta" e o dado agora sobrevive ao ciclo completo, alimentando o detalhe de assiduidade e o relatório.'},
        {tipo:'correcao', texto:'O traço ("–") usado na planilha para andar e atividades vazios voltava como se fosse um andar ou uma atividade de verdade.'},
        {tipo:'melhoria', texto:'Linhas antigas da planilha (sem a coluna nova) continuam sendo lidas normalmente.'},
      ]
    },
    {
      v: '1.6.3',
